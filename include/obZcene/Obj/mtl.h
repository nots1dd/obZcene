...
extern "C"
{
  ...
  inline static int __OBZ_find_material(const OBZ_ObjMesh* mesh, const char* name)
  {
    ...
    {
      const OBZ_Material* m = (const OBZ_Material*)obz_arr_get_const(&mesh->materials, i);
  ...
  inline static int __OBZ_push_material(OBZ_ObjMesh* mesh, const OBZ_Material* mat)
  {
    if (!mesh || !mat)
      return -1;
  ...
  inline static OBZ_Result obz_mtl_load(const char* mtl_path, OBZ_ObjMesh* out)
  {
    if (!mtl_path || !out)
    ...
    cur.Ns = 0.0f;
    cur.Kd = (Vec3){1.0f, 1.0f, 1.0f};
    cur.Ka = (Vec3){0.0f, 0.0f, 0.0f};
    cur.Ks = (Vec3){0.0f, 0.0f, 0.0f};
