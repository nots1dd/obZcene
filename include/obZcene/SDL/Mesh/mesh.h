...
extern "C"
{
  ...
  inline static OBZ_Mesh3D* obz_mesh_from_obj(const OBZ_ObjMesh* src)
  {
    ...
    if (out_mesh->noOfVerts)
      out_mesh->verts =
        memcpy(obz_malloc(sizeof(Vec3) * out_mesh->noOfVerts),
               (Vec3*)obz_arr_get_data_const(&src->positions), sizeof(Vec3) * out_mesh->noOfVerts);

    if (out_mesh->noOfUVs)
      out_mesh->uvs =
        memcpy(obz_malloc(sizeof(Vec2) * out_mesh->noOfUVs),
               (Vec2*)obz_arr_get_data_const(&src->texcoords), sizeof(Vec2) * out_mesh->noOfUVs);

    if (out_mesh->noOfNorms)
      out_mesh->norms =
        memcpy(obz_malloc(sizeof(Vec3) * out_mesh->noOfNorms),
    ...
    if (out_mesh->material_count > 0)
      out_mesh->materials = memcpy(obz_malloc(sizeof(OBZ_Material) * out_mesh->material_count),
                                   (OBZ_Material*)obz_arr_get_data_const(&src->materials),
                                   sizeof(OBZ_Material) * out_mesh->material_count);
