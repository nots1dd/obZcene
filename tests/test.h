...
static inline void OBZ_stats_add(OBZ_stats_t *s, double diff) {
    double a = fabs(diff);
    s->sum_abs += a;
    s->sum_sq += a * a;
...
#define BENCH_SIMD_TYPE(TYPE, FUNC_REF, FUNC_SIMD, ARGS, NAME, RUNS, xs, out, N, t_ref) do { \
    ...
    for (int r = 0; r < RUNS; r++)                                                         \
        FUNC_SIMD ARGS;                                                                     \
    double t_simd = OBZ_now_s() - t0;                                                      \
                                                                                             \
    /* compute errors */                                                                    \
    for (int i = 0; i < N; i++)                                                            \
        OBZ_stats_add(&s, (double)(FUNC_REF(xs[i]) - out[i]));                             \
                                                                                             \
...
*/
#define OBZ_BENCH_COMPARE(func1, func2, xs, out, N, RUNS) do { \
...
\
    ...
    for (int r=0;r<RUNS;r++) \
        for (int i=0;i<(N);i++) \
            out[i] = func2(xs[i]); \
    t2_test = OBZ_now_s() - t0; \
\
    /* compute errors */ \
    OBZ_stats_reset(&s); \
    for (int i=0;i<(N);i++) \
        OBZ_stats_add(&s, (double)func1(xs[i]) - (double)out[i]); \
\
